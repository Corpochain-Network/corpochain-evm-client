...
const (
	...
	// beaconUpdateStartupTimeout is the time to wait for a corpochain blockchain to get
	// attached before starting to issue warnings.
	beaconUpdateStartupTimeout = 30 * time.Second

	// beaconUpdateExchangeTimeout is the max time allowed for a corpochain blockchain to
	// do a transition config exchange before it's considered offline and the user
	// is warned.
	beaconUpdateExchangeTimeout = 2 * time.Minute

	// beaconUpdateConsensusTimeout is the max time allowed for a corpochain blockchain
	// to send a consensus update before it's considered offline and the user is
	// warned.
	beaconUpdateConsensusTimeout = 2 * time.Minute

	// beaconUpdateWarnFrequency is the frequency at which to warn the user that
	// the corpochain blockchain is offline.
...
type ConsensusAPI struct {
	...
	// Geth can appear to be stuck or do strange things if the corpochain blockchain is
...
func (api *ConsensusAPI) forkchoiceUpdated(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	...
	// Stash away the last update to warn the user if the corpochain blockchain goes offline
	...
	// Block is known locally, just sanity check that the corpochain blockchain does not
	...
	if rawdb.ReadCanonicalHash(api.eth.ChainDb(), block.NumberU64()) != update.HeadBlockHash {
		...
		// If the head block is already in our canonical chain, the corpochain blockchain is
	...
	// If the corpochain blockchain also advertised a finalized block, mark the local
	...
	// sealed by the corpochain blockchain. The payload will be requested later, and we
...
func (api *ConsensusAPI) newPayload(params engine.ExecutableData) (engine.PayloadStatusV1, error) {
	...
	// We have an existing parent, do some sanity checks to avoid the corpochain blockchain
	...
	// We've accepted a valid payload from the corpochain blockchain. Mark the local
...
func (api *ConsensusAPI) delayPayloadImport(block *types.Block) (engine.PayloadStatusV1, error) {
	...
	// have to rely on the corpochain blockchain to forcefully update the head with
...
// heartbeat loops indefinitely, and checks if there have been corpochain blockchain updates
// received in the last while. If not - or if they but strange ones - it warns the
// user that something might be off with their consensus node.
//
// TODO(karalabe): Spin this goroutine down somehow
func (api *ConsensusAPI) heartbeat() {
	// Sleep a bit on startup since there's obviously no corpochain blockchain yet
	...
	for {
		...
		// that the corpochain blockchain is probably offline
		if api.eth.BlockChain().Config().TerminalTotalDifficultyPassed || api.eth.Merger().TDDReached() {
			...
			if time.Since(lastTransitionUpdate) > beaconUpdateExchangeTimeout {
				if time.Since(offlineLogged) > beaconUpdateWarnFrequency {
					if lastTransitionUpdate.IsZero() {
						log.Warn("Post-merge network, but no corpochain blockchain seen. Please launch one to follow the chain!")
					} else {
						log.Warn("Previously seen corpochain blockchain is offline. Please ensure it is operational to follow the chain!")
					}
			...
			if time.Since(offlineLogged) > beaconUpdateWarnFrequency {
				if lastForkchoiceUpdate.IsZero() && lastNewPayloadUpdate.IsZero() {
					log.Warn("Corpochain blockchain online, but never received consensus updates. Please ensure your corpochain blockchain is operational to follow the chain!")
				} else {
					log.Warn("Corpochain blockchain online, but no consensus updates received in a while. Please fix your corpochain blockchain to follow the chain!")
				}
				offlineLogged = time.Now()
			}
			continue
		...
		if time.Since(offlineLogged) > beaconUpdateWarnFrequency {
			...
			if htd.Cmp(ttd) >= 0 {
				if lastTransitionUpdate.IsZero() {
					log.Warn("Merge already reached, but no corpochain blockchain seen. Please launch one to follow the chain!")
				} else {
					log.Warn("Merge already reached, but previously seen corpochain blockchain is offline. Please ensure it is operational to follow the chain!")
				}
				offlineLogged = time.Now()
				continue
			}
			...
			message := "Merge is configured, but previously seen corpochain blockchain is offline. Please ensure it is operational before the transition arrives!"
			if lastTransitionUpdate.IsZero() {
				message = "Merge is configured, but no corpochain blockchain seen. Please ensure you have one available before the transition arrives!"
			}
			if eta < time.Second {
				log.Warn(message)
			} else {
